...
// =====================================================
// ITEMS SUB-ROUTER (mounted at /items)
// =====================================================

// @route   GET /api/menus/items/:id
// @desc    Get single menu item
itemsRouter.get('/:id', async (req, res) => {
  console.log('GET /api/menus/items/:id route hit with id:', req.params.id);
  try {
...
itemsRouter.put('/:id', auth, async (req, res) => {
  try {
    const { parent_id, title, url, target, icon, sort_order } = req.body;

    // Use title field (current database schema)
    const finalTitle = title || '';

    await db.query(`
      UPDATE menu_items SET
        title = ?, url = ?, target = ?, icon = ?,
        parent_id = ?, sort_order = ?
      WHERE id = ?
    `, [finalTitle, url, target || '_self', icon || null,
...
// Mount items router FIRST (before any :id routes)
...
router.get('/location/:location', async (req, res) => {
  try {
    const [menus] = await db.query(`
      SELECT * FROM menus WHERE location = ? LIMIT 1
    `, [req.params.location]);
