...
router.get('/', optionalAuth, async (req, res) => {
  try {
    ...
    // Filter by type if specified (supports comma-separated list or 'all' for no filter)
    if (type && type !== 'all') {
      if (type.includes(',')) {
        const types = type.split(',').map(t => t.trim());
        whereConditions.push(`p.type IN (${types.map(() => '?').join(',')})`);
        params.push(...types);
      } else {
...
router.get('/:idOrSlug', optionalAuth, async (req, res) => {
  try {
    ...
    const [posts] = await db.query(query, [idOrSlug]);
