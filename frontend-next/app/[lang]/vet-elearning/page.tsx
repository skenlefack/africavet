...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const titles = {
    fr: 'VET E-Learning | Formation One Health Cameroun',
    en: 'VET E-Learning | One Health Cameroon Training',
  };
...
export default async function VetElearningPage({ params }: PageProps) {
  const { lang } = await params;
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              {categories.filter(c => c.is_active).map((category, index) => {
                ...
                return (
                  <Link
                    key={category.id}
                    href={`/${lang}/vet-elearning/courses?category=${category.slug}`}
        ...
        <section className="py-12 px-[5%]">
          <div className="max-w-6xl mx-auto">
            <div className="flex items-center justify-between mb-8">
              ...
              <Link href={`/${lang}/vet-elearning/paths`}>
