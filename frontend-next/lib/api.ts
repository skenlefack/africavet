...
export async function getPosts(options?: {
  ...
  // Only add type filter if explicitly specified
  if (options?.type) {
...
export async function getFeaturedPosts(limit: number = 6): Promise<ApiResponse<Post[]>> {
  return getPosts({ status: 'published', limit, featured: true, sort: 'published_at', order: 'DESC', type: 'news' });
}
...
export async function getVetAlerts(options?: {
  page?: number;
  limit?: number;
  status?: string;
  priority?: string;
  ...
  const query = params.toString() ? `?${params.toString()}` : '';
  return fetchApi<VetAlert[]>(`/vet-alerts${query}`);
}

export async function getVetAlert(id: number): Promise<ApiResponse<VetAlert>> {
  return fetchApi<VetAlert>(`/vet-alerts/${id}`);
}

export async function getRecentVetAlerts(limit: number = 6): Promise<ApiResponse<VetAlert[]>> {
  return getVetAlerts({ status: 'approved', limit });
}
...
export async function getOpportunity(slug: string): Promise<ApiResponse<Opportunity>> {
  return fetchApi<Opportunity>(`/opportunities/${slug}`);
}

export async function getFeaturedOpportunities(limit: number = 6): Promise<ApiResponse<Opportunity[]>> {
  return getOpportunities({ status: 'published', is_featured: true, limit });
}

export async function getRecentOpportunities(limit: number = 10): Promise<ApiResponse<Opportunity[]>> {
  return getOpportunities({ status: 'published', limit });
}
